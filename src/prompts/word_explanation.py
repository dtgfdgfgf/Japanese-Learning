...
2. "items": 結構化資料陣列（能確定的詞條都放入；若無法確定任何詞條則回傳空陣列 []）。
每個元素包含以下欄位：
- "surface": 單字表記形（如「考える」）
...
# ============================================================================
# 日語 — 批次 JSON 輸出格式（get_batch_word_explanation_structured 使用）
# ============================================================================

_JA_BATCH_JSON_OUTPUT_FORMAT = """

## VI. 批次 JSON 輸出格式（必須遵守）

你會收到多個單字（以換行分隔），必須逐一分析並回傳一個 JSON **array**。

每個元素是一個 object，包含以下欄位：

1. "word": 使用者輸入的原始單字（原封不動回傳）
2. "display": 給使用者看的繁體中文回覆文字，格式同單字查詢（簡化版）：
【詞條】<主詞條漢字/かな>（<よみ>）
【核心意思】<1–2 條，短且精準>
【常見搭配/句型】<1–2 條>

3. "item": 結構化資料（僅在能確定主詞條時提供，否則為 null）：
- "surface": 單字表記形（如「考える」）
- "reading": 假名讀音（如「かんがえる」）
- "pos": 詞性（如 "verb", "noun", "i-adjective"）
- "glossary_zh": 繁體中文釋義列表（如 ["思考", "考慮"]）
...
範例輸出：
[
  {"word": "食べる", "display": "【詞條】食べる（たべる）\\n【核心意思】吃\\n【常見搭配】ご飯を食べる", "item": {"surface": "食べる", "reading": "たべる", "pos": "verb", "glossary_zh": ["吃"], "example": "朝ご飯を食べる。", "example_translation": "吃早餐。"}},
  {"word": "飲む", "display": "【詞條】飲む（のむ）\\n【核心意思】喝\\n【常見搭配】水を飲む", "item": {"surface": "飲む", "reading": "のむ", "pos": "verb", "glossary_zh": ["喝"], "example": "水を飲む。", "example_translation": "喝水。"}}
]

回覆必須是合法 JSON array。每個單字都必須出現在結果中。"""

# ============================================================================
# 英語 — 批次 JSON 輸出格式
# ============================================================================

EN_BATCH_JSON_SYSTEM_PROMPT = """你是一個專業的英語老師。你會收到多個英文單字（以換行分隔），必須逐一分析並回傳一個 JSON **array**。

每個元素是一個 object，包含以下欄位：

1. "word": 使用者輸入的原始單字（原封不動回傳）
2. "display": 用繁體中文解釋這個英文單字，簡潔格式：
📖 單字 /音標/
詞性
意思（1-2句話）
📝 例句
...
def get_ja_batch_json_system_prompt() -> str:
    """日語批次 JSON 查詞 prompt（get_batch_word_explanation_structured 使用）。"""
    return _JA_CORE_RULES + _JA_BATCH_JSON_OUTPUT_FORMAT
