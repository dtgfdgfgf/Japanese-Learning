...
1. **save**: 用戶想要保存{lang_name}學習素材
   - 例如：單一{lang_name}單字（如 "try"、"考える"）、句子、文章、單字列表
   - 通常是{lang_name}內容，沒有明確的指令詞
   - 單一{lang_name}詞彙是最常見的情況，應積極判斷為 save

2. **practice**: 用戶想要練習
   - 例如：「我要練習」、「來個測驗」、「複習」

3. **search**: 用戶想要搜尋某個詞彙或文法
   - 例如：「找一下XXX」、「XXX是什麼意思」
...
```json
{{
  "intent": "save|practice|search|delete|help|chat|unknown",
  "confidence": 0.0-1.0,
...
- 單一{lang_name}詞彙（無論長短）應判斷為 save，confidence 給 0.85 以上
