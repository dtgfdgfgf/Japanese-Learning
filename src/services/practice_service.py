...
class PracticeService:
    ...
    async def create_session(
    ...
    ) -> tuple[Optional[PracticeSession], str]:
        """
        ...
        if total_items < MIN_ITEMS_FOR_PRACTICE:
            return None, (
                f"你的題庫還不夠 📚\n"
                f"目前只有 {total_items} 個項目\n"
                f"請先入庫更多素材（至少需要 {MIN_ITEMS_FOR_PRACTICE} 個）"
            )
        ...
        # Store session
        _sessions[user_id] = practice_session
    ...
    async def _select_items_for_practice(
        ...
        user_id: str,
        count: int,
        criteria: Optional[ItemSelectionCriteria] = None,
    ) -> list:
        """
        ...
        recent_cutoff = datetime.utcnow() - timedelta(hours=criteria.recent_hours)
    ...
    def _generate_question(self, item) -> Optional[PracticeQuestion]:
        """
    ...
    async def submit_answer(
        ...
        user_id: str,
        answer_text: str,
    ) -> tuple[Optional[PracticeAnswer], str]:
        """
        ...
        if not session:
            return None, "沒有進行中的練習，請先輸入「練習」開始"
...
def get_active_session(user_id: str) -> Optional[PracticeSession]:
    """Get active practice session for a user."""
...
def clear_session(user_id: str) -> None:
    """Clear a user's practice session."""
