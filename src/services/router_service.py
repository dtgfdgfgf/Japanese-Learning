...
- 鼓勵學習"""
    ...
    async def get_word_explanation(
        ...
        """
        if target_lang == "ja":
            system_prompt = """你是一個專業的日語老師。
⚠️ 嚴格遵守使用者輸入的假名：不可自動修正或替換。注意大小假名的區別（拗音），例如「きゆう」(ki-yu-u) 與「きゅう」(kyuu) 是不同的詞，分別對應不同的漢字。若輸入的假名不對應任何已知單字，仍以使用者輸入為準回答，並標註該讀法不常見或可能的正確拼法。
