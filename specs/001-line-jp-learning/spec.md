...
**目標**：提供一個透過 LINE 即可完成「素材入庫 → 結構化分析 → 練習複習」完整循環的個人化語言學習助手（支援日文與英文）。

---

## Actors

- **語言學習者**：主要使用者，透過 LINE 貼入日文或英文素材、觸發分析與練習
...
1. **Given** 使用者已加入 LINE Bot 好友，**When** 使用者貼上一段日文「考える」並輸入「入庫」，**Then** 系統回覆「已入庫：考える」（截斷預覽）並將素材存入資料庫
2. **Given** 使用者未貼上任何訊息，**When** 使用者直接輸入「入庫」，**Then** 系統回覆「請先貼上要入庫的內容」
3. **Given** 使用者貼上非支援語言內容（如純中文），**When** 內容透過 Router，**Then** 系統回覆「目前支援日文和英文內容」
4. **Given** 使用者輸入短單字（如「apple」），**When** Router 分類為 SAVE intent，**Then** 系統以 LLM 解釋該單字並詢問是否入庫（pending_save 流程，5 分鐘 TTL）
5. **Given** 使用者輸入「鋭い save」，**When** 匹配 WORD_SAVE 模式，**Then** 系統直接入庫該單字（不經 LLM 解釋）
...
1. **Given** 使用者有已入庫的 items，**When** 使用者輸入「刪除 食べる」且只有 1 筆匹配，**Then** 系統將該 item 標記 is_deleted=true 並回覆「已刪除「食べる」」
2. **Given** 使用者輸入「刪除 食」且有 2-5 筆匹配，**When** 訊息送出，**Then** 系統顯示編號列表，使用者輸入編號選擇刪除
3. **Given** 使用者輸入「清空資料」，**When** 訊息送出，**Then** 系統回覆「確定要清空所有資料嗎？請在 60 秒內回覆『確定清空資料』」
4. **Given** 系統等待二次確認，**When** 使用者回覆「確定清空資料」，**Then** 系統將該 user 所有資料軟刪除並回覆刪除明細
...
- **非支援語言內容**：使用者貼入非日文/英文內容時，系統提示「目前支援日文和英文內容」
- **練習中途離開**：使用者收到題目後未作答就離開，session 30 分鐘自動過期，下次「練習」重新出題
- **短單字查詢**：使用者直接輸入短單字時，Router 判斷 intent=save → LLM 解釋 + 詢問入庫（5 分鐘 pending TTL）
- **超長文本**：超過 2000 字的文本跳過 Router，直接入庫以節省 LLM tokens
...
- **FR-006**: 系統必須支援「刪除 <keyword>」指令，搜尋匹配 items 後軟刪除（1 筆直接刪，2-5 筆列表選擇，>5 筆提示更精確）
- **FR-007**: 系統必須支援「清空資料」指令，需二次確認（60 秒 TTL）後軟刪除該 user 所有資料
- **FR-008**: 系統必須支援「隱私」指令，回覆資料保存與使用說明
- **FR-021**: 系統必須支援「<單字> save」指令，直接入庫指定單字（不經 LLM 解釋）
- **FR-022**: 系統必須支援「統計」/「進度」指令，顯示學習進度（素材數、練習統計、7 日趨勢）
- **FR-023**: 系統必須支援「用量」/「cost」指令，顯示 API 用量（本月 + 累計，按 model 分組）
- **FR-024**: 系統必須支援模式切換（免費/便宜/嚴謹），切換 LLM provider
- **FR-025**: 系統必須支援目標語言切換（日文/英文），影響 Extractor 和 Practice 行為
- **FR-026**: 系統必須支援「結束練習」指令，中途清除 practice session
...
- 資料匯出功能（CSV/JSON export）
- 圖形化統計面板（Web dashboard）

---

## Assumptions

- 使用者為語言學習者（日文或英文），對指令式互動接受度高
- 使用者已有 LINE 帳號且熟悉 LINE Bot 基本操作
- LLM API（Anthropic Claude / Google Gemini）可穩定提供服務
...
|---------|------|---------|
| 1.1.0 | 2026-02-12 | 同步實作：英文支援、刪除 <keyword> 取代刪除最後一筆、新增 FR-021~026（word save / 統計 / 用量 / 模式切換 / 語言切換 / 結束練習）、更新 Out of Scope、更新 Edge Cases |
